...
public class Data_Centre_Router extends JFrame implements ActionListener {
	...
	JButton attacker = new JButton("View Attackers");
	JButton sub = new JButton("View Digital Sign");
	...
	Data_Centre_Router() {
		...
		attacker.setBounds(320, 500, 150, 30);
		...
		int[] ports = new int[] { 8009,8001, 9009 };
	...
	public static void main(String[] args) {
		Data_Centre_Router r = new Data_Centre_Router();
		r.setSize(900, 600);
		r.setTitle("Data Centre Router1::An Efficient Online Algorithm for Dynamic SDN Controller Assignment in Data Center Networks");
		r.setVisible(true);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == attacker) {
			...
			ViewAttackers vw = new ViewAttackers();
			vw.setTitle("View All Attackers");
			vw.setSize(500, 400);
			vw.setVisible(true);
			vw.setResizable(false);
		...
		if (e.getSource() == sub) {
			...
			View_Digitalsign vw = new View_Digitalsign();
			...
			if (this.port == 8009) {
				...
				try {
					...
					while (true) {
						...
						String atype = "Content Attack";

						DBCon
								.getConnection()
								.createStatement()
								.executeUpdate(
										"insert into Attacker values( '"+ node+ "' , '"+ atype + "', '"+ dt+ "' ) ");
					...
					while (true) {
						...
						.executeUpdate(
								"update Bandwidth set Bandwidth = '"
										+ bw + "' where NodeName = '" + node
										+ "'  ");

						
					
						
						String atype = "BW Attack";

						DBCon
								.getConnection()
								.createStatement()
								.executeUpdate("insert into Attacker values( '"+ node+ "' , '"+ atype + "', '"+ dt+ "') ") ;
			...
			if (this.port == 9009) {
				...
				try {
					...
					while (true) {
							...
							if (bw1 < nb) {
								...
								int x = JOptionPane
										.showConfirmDialog(null,
												"Faulty Node Detected due to Jamming occurs in Node1 \n Do you want Alter the Path  ");
								...
								if (x == 0) {
									...
									if (bw2 < nb) {
										...
										int x1 = JOptionPane
												.showConfirmDialog(null,
														"Faulty Node Detected due to Jamming occurs in Node2 \n Do you want Alter the Path  ");
										...
										if (x1 == 0) {
											...
											if (bw3 < nb) {
												...
												int approv = JOptionPane
														.showConfirmDialog(
																null,
																"Faulty Node Detected due to Jamming occurs in Node-3");
										...
										if (bw5 < nb) {
											...
											int app = JOptionPane
													.showConfirmDialog(null,
															"Faulty Node Detected due to Jamming occurs in Node-5");
											...
											if (bw7 < nb) {
												...
												int option = JOptionPane
														.showConfirmDialog(
																null,
																"Faulty Node Detected due to Jamming occurs in Node7 \n Do you want to Alter the path ");
						...
						if (action.equals("CASE2")) {
							...
							while (rs.next() == true) {
								...
								if (node.equals("N2")
									...
									int x = JOptionPane
											.showConfirmDialog(null,
													"Attaker Found due to Faulty Node Detected , \n Do you want to Secure the file");
						...
						double th=((timeDelay/content.length())*1024);
						...
						String sin2="insert into Results values('"+fname+"','"+String.valueOf(timeDelay)+"','"+String.valueOf(th)+"','"+dt+"')";
